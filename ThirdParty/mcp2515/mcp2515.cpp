...
MCP2515::ERROR MCP2515::reset(void)
{
	...
	modifyRegister(MCP_RXB0CTRL, RXBnCTRL_RXM_MASK | RXB0CTRL_BUKT,
...
void MCP2515::readRegisters(const REGISTER reg, uint8_t values[],
        const uint8_t n)
{
	startSPI();
	SPI.transfer(INSTRUCTION_READ);
...
void MCP2515::setRegisters(const REGISTER reg, const uint8_t values[],
        const uint8_t n)
{
	startSPI();
	SPI.transfer(INSTRUCTION_WRITE);
...
void MCP2515::modifyRegister(const REGISTER reg, const uint8_t mask,
        const uint8_t data)
{
	startSPI();
...
MCP2515::ERROR MCP2515::setFilterMask(const MASK mask, const bool ext,
...
MCP2515::ERROR MCP2515::setFilter(const RXF num, const bool ext,
        const uint32_t ulData)
{
	ERROR res = setConfigMode();
	if (res != ERROR_OK)
